...
async function runDev(hostEntry, trigger, timeout = '50', extraEnv = {}, launcher = 'scripts/dev.mjs') {
  ...
  child.stdout.on('data', async (chunk) => {
    ...
    for (const match of stdout.matchAll(/DESCENDANT_ID (\d+) (\d+)/g)) {
      ...
      if (!recordedIds.has(pid)) {
        recordedIds.add(pid);
        pids.push({ pid, startTime: match[2] });
      }
    }
    ...
    if (match && !recorded) {
      ...
      for (const raw of match.slice(1)) {
        ...
        pids.push({ pid, ...(await identity(pid)) });
      }
    }
    if (!signalled && trigger?.(stdout)) {
      signalled = true;
      child.kill('SIGTERM');
    }
  });
  ...
  const code = await new Promise((resolve, reject) => {
    const timer = setTimeout(() => {
      child.kill('SIGKILL');
      reject(new Error('dev cleanup test timed out'));
    }, 15_000);
...
const host = await runDev('scripts/fixtures/wedged-descendants.mjs', (stdout) =>
  stdout.includes('Podcaster readiness:'),
);
...
const hmrHost = await runDev(
  'scripts/fixtures/wedged-descendants.mjs',
  (stdout) => stdout.includes('[backend] internal host:'),
  '50',
  {},
  'scripts/dev-hmr.mjs',
);
...
const build = await runDev('unused', (stdout) => stdout.includes('BUILD_WEDGED'), '50', {
  PODCASTER_WEB_BUILD_ENTRY: 'scripts/fixtures/wedged-descendants.mjs',
  FIXTURE_MODE: 'build',
});
...
process.stdout.write(
  'dev cleans stubborn descendants after interruption and spontaneous leader exits; startup failure: passed\n',
);
