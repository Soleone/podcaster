...
# These names and hashes are the complete immutable CustomVoice snapshot
# acquired by scripts/acquire-qwen3-tts.py.  Checking every file prevents a
# valid primary safetensors file from being paired with a different tokenizer
# or speech codec.
...
def _verify_qwen_assets(
    ...
    for relative, expected in MODEL_ASSETS:
...
class Qwen3StreamingAdapter:
    ...
    def synthesize_stream(
        ...
        with self._lock:
            ...
            native_voice = self._native_voices[selected_voice]
            self._active = True
