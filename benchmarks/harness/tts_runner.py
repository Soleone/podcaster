...
def _verified_tts_config(
    ...
    if (
        ...
        or prompts.get("language") != config.get("language")
        or prompts.get("id") != config.get("promptManifestId")
        or len(prompts["items"]) < 20
    ):
    ...
    voices_file = next(
        (entry for entry in model["files"] if entry["path"] == model.get("voicesPath")), None
    )
    exact_pairs["voicesSha256"] = voices_file.get("sha256") if voices_file else None
    ...
    voices_path = (ROOT / model["voicesPath"]).resolve()
    if (
        str((ROOT / config["modelPath"]).resolve()) != str(model_path)
        or str((ROOT / config["voicesPath"]).resolve()) != str(voices_path)
    ):
        raise ValueError("TTS configured paths do not match verified manifest paths")
    config["modelPath"] = str(model_path)
    config["voicesPath"] = str(voices_path)
    return config, prompts, prompts_hash, model


def _comparison_semantics(config: dict[str, Any], prompts_hash: str) -> dict[str, Any]:
    return {
        "kind": "tts",
...
def compare_tts_runs(run_dirs: list[Path]) -> dict[str, Any]:
    ...
    for path in run_dirs:
        ...
        if run.get("kind") != "tts" or not isinstance(run.get("comparisonSemantics"), dict):
    ...
    for path, run, _ in loaded[1:]:
        ...
        if differing or run["comparisonSemanticsSha256"] != loaded[0][1][
...
def _playback_paced(
    adapter: KokoroStreamingAdapter, text: str, cancel: CancelToken
) -> dict[str, Any]:
    ...
    def enqueue(chunk: Any) -> None:
        ...
        while True:
            ...
            if not playback_start_ns:
                playback_start_ns.append(queued_ns + PLAYBACK_BUFFER_MS * 1_000_000)
            record["queuedMonotonicNs"] = queued_ns
...
def run_tts(
    ...
    adapter_factory: Callable[[], KokoroStreamingAdapter] = KokoroStreamingAdapter,
) -> Path:
    ...
    semantics = _comparison_semantics(config, prompts_hash)
    run = {
        "schemaVersion": 1,
        "runId": run_id,
        "kind": "tts",
        ...
        "models": [
            {
                ...
                "voice": config["candidate"]["voice"],
                "provider": config["candidate"]["provider"],
                "nativeSampleRate": config["nativeSampleRate"],
                "modelSha256": config["candidate"]["modelSha256"],
                "voicesSha256": config["candidate"]["voicesSha256"],
                "runtimeRevision": config["candidate"]["runtimeRevision"],
                "onnxReleaseRevision": config["candidate"]["onnxReleaseRevision"],
            }
        ],
        "configId": config["id"],
    ...
    adapter: KokoroStreamingAdapter | None = None
    ...
    if adapter is not None and primary_stage is None and soak_minutes > 0:
        soak_adapter: KokoroStreamingAdapter | None = None
    ...
    (run_dir / "README.md").write_text(
        f"# Kokoro TTS benchmark\n\nRerun: `{' '.join(run['command'])}`\n\n"
        f"Validate: `uv run python -m benchmarks.harness validate {run_dir}`\n\n"
        "CPUExecutionProvider is intentional and no candidate GPU acceleration is claimed. "
        "Output is signed little-endian PCM16 mono at 24 kHz in WAV containers. TTFA begins at "
        "tts_requested and ends at the first accepted non-empty PCM chunk. RTF is synthesis "
        "processing seconds divided by generated audio seconds. Gain is fixed at 0.9 with no "
        "per-item normalization or resampling. Soak mode consumes a bounded queue at playback pace.\n"
    )
...
def probe_kokoro_cancellation(
    ...
    config, prompts, prompts_hash, model = _verified_tts_config(
        "kokoro", config_path, prompts_path
    )
    ...
    if run_dir is not None:
        ...
        if (
            ...
            or attached_run.get("models", [{}])[0].get("sha256") != model["sha256"]
        ):
    ...
    try:
        ...
        poisoned = adapter._poisoned
        adapter.close()
    post_close_survivors = _worker_names()
    result = {
        "schemaVersion": 1,
        "probe": "kokoro-cancellation-after-first-audio-v2",
