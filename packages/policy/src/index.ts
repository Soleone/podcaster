...
export interface PriorPolicyDecision {
  turnId: string;
  eligible: boolean;
  posture: Posture;
}
...
function explicitInvitation(value: string): boolean {
  return /\b(?:what do you think|what['’]s your take|what is your take|any thoughts|can you|could you|would you|do you agree|please (?:respond|tell me)|(?:respond|reply|answer) please)\b/iu.test(
    value,
  );
}
...
export function decide(input: PolicyInput): PolicyDecision {
  ...
  const silence = (reason: PolicyReasonCode, eligible = false): PolicyDecision => ({
  ...
  const challengeAllowed =
    input.persona.challenge_enabled &&
    input.stableUserTurnCount >= 2 &&
    input.eligibleTurnsSinceChallenge >= CHALLENGE_COOLDOWN_TURNS;
  ...
  for (const choice of choices) {
    if (bucket < choice.weight) {
      selected = choice.posture;
      break;
    }
