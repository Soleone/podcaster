...
describe("persona v1 parser", () => {
  ...
  it("rejects invalid UTF-8 and body byte overflow", () => {
    expect(parsePersona(Uint8Array.from([0xc3, 0x28]))).toMatchObject({ ok: false, errors: [{ code: "invalid_utf8" }] });
    expect(parsePersona("x".repeat(16 * 1024 + 1))).toMatchObject({ ok: false, errors: expect.arrayContaining([expect.objectContaining({ code: "body_too_large" })]) });
  });
