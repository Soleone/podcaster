...
test('streams a dimmed assistant preview before audio and solidifies it on final', async ({ page, origin }) => {
  ...
  await emit(page, 'transcript.final', {
    turnId: 'turn-1',
    text: 'Tell me about perceived latency',
  ...
  await emit(page, 'reasoning.delta', {
    turnId: 'turn-1',
    responseId: 'response-1',
    text: 'Perceived latency drops when text streams',
  });
  ...
  await emit(page, 'reasoning.final', {
    ...
    responseId: 'response-1',
    posture: 'question',
    text: 'Perceived latency drops when text streams first.',
  });
  await expect(page.locator('.assistant-bubble.tentative')).toHaveCount(0);
  await expect(
    page.locator('.assistant-bubble', { hasText: 'Perceived latency drops when text streams first.' }),
  ).toBeVisible();
...
test('drops a streaming preview when the response fails before finalizing', async ({ page, origin }) => {
  ...
  await emit(page, 'reasoning.delta', {
    turnId: 'turn-1',
    responseId: 'response-1',
    text: 'A preview that will not land',
  });
