...
export interface EncodeProgress { requestId: number; progress: number }
...
workerScope.onmessage = (event: MessageEvent<EncodeRequest>) => {
  ...
  const onProgress = reportProgress ? (fraction: number) => workerScope.postMessage({ requestId, progress: fraction } satisfies EncodeProgress) : undefined;
  try {
