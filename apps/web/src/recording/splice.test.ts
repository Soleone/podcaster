...
function item(
  partial: Partial<StoredRecordingItem> & {
    itemId: string;
    role: RecordingRole;
    recordSeq: number;
...
async function deps(
  decoded: Array<{ sampleRate: number; channelData: Float32Array }>,
  ...
  const resample = vi.fn(
    options.resample ??
      ((data: Float32Array, fromRate: number, toRate: number) => {
...
describe('buildRecording', () => {
  it('orders items by turn timeline (user before agent), trims interrupted agents at deliveredSamples, and inserts the gap', async () => {
    ...
    expect(resample.mock.calls.map((call) => [call[1], call[2]])).toEqual([
      [16000, FINAL_SAMPLE_RATE],
      [24000, FINAL_SAMPLE_RATE],
      [16000, FINAL_SAMPLE_RATE],
      [24000, FINAL_SAMPLE_RATE],
    ]);
    ...
    const segments = [
      Math.round((1600 * FINAL_SAMPLE_RATE) / 16000),
      Math.round((100 * FINAL_SAMPLE_RATE) / 24000),
      Math.round((800 * FINAL_SAMPLE_RATE) / 16000),
      Math.round((1200 * FINAL_SAMPLE_RATE) / 24000),
    ];
  ...
  it('honors a custom gap and resamples each item', async () => {
    ...
    expect(encode.mock.calls[0]![0].length).toBe(
      Math.round((1600 * FINAL_SAMPLE_RATE) / 16000) + Math.round((800 * FINAL_SAMPLE_RATE) / 16000) + gap,
    );
    store.close();
  });
  ...
  it('leaves near-silent segments unchanged instead of amplifying them toward clipping', async () => {
    ...
    const first = await deps([{ sampleRate: 16000, channelData: signal }], turns, {
      store,
      resample: identityResample,
    });
    await buildRecording(SESSION, {
      store,
      turns: first.turns,
      decode: first.decode,
      resample: first.resample,
      encode: first.encode,
      gapMs: 0,
    });
    const second = await deps([{ sampleRate: 16000, channelData: signal.slice() }], turns, {
      store,
      resample: identityResample,
    });
    await buildRecording(SESSION, {
      store,
      turns: second.turns,
      decode: second.decode,
      resample: second.resample,
  ...
  it('caps upward gain at MAX_NORMALIZATION_GAIN instead of reaching the target', async () => {
    ...
    const { decode, resample, encode, turns } = await deps(
      [{ sampleRate: 16000, channelData: signal }],
      [turn(T1, 1)],
      { store, resample: identityResample },
    );
    await buildRecording(SESSION, { store, turns, decode, resample, encode, gapMs: 0 });
    const pcm = encode.mock.calls[0]![0];
  ...
  it('passes exact pre-change PCM16 to the encoder in none mode', async () => {
    ...
    const { decode, resample, encode, turns } = await deps(
      [{ sampleRate: 16000, channelData: fixture }],
      [turn(T1, 1)],
      { store, resample: identityResample },
    );
  ...
  it('counts a survivor that decodes to no samples toward per-clip progress', async () => {
    ...
    await store.put(item({ itemId: 'b', role: 'user', recordSeq: 1, sampleRate: 16000, turnId: T1 }));
    const progress: ExportProgress[] = [];
    const blob = await buildRecording(SESSION, {
      store,
      turns,
      decode,
      resample,
  ...
  it('maps encoder fractions into the encoding band and never regresses', async () => {
    ...
    const { decode, resample, turns } = await deps([{ sampleRate: 16000, channelData: tone(1600) }], [turn(T1, 1)], {
      store,
      resample: identityResample,
    });
    const progress: ExportProgress[] = [];
