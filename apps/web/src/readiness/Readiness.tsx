...
export function Readiness() {
  ...
  return <main>
    ...
    {!acknowledged ? <section aria-labelledby="privacy-title" className="card">
      ...
      <ul className="readiness-list">{snapshot?.capabilities.map(row => <li key={row.id}><div><strong>{row.label}</strong><span className={`badge ${row.state}`}>{row.state === 'needs_action' ? 'Needs attention' : 'Ready'}</span></div><p>{row.reason}</p><p><strong>Next:</strong> {row.action}</p></li>)}</ul>
      <button type="button" disabled title="Microphone capture is intentionally not implemented in Milestone 0">Enable microphone</button>
      <p className="hint">Microphone access is a later milestone. No permission prompt or capture occurs in this readiness stub.</p>
