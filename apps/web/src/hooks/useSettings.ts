...
export function useSettings({
  ...
  const setSettingsModel = useCallback(
    (model: SettingsModel) => {
  ...
  if (!settingsReadyRef.current) {
    settingsReadyRef.current = new Promise<void>((resolve) => {
      resolveSettingsReadyRef.current = resolve;
    });
  }
  ...
  const mergeCustomModels = useCallback(
    (models: TtsModelDescriptor[]): TtsModelDescriptor[] =>
      models.map((item) => {
  ...
  const syncStoredCustomVoices = useCallback(
    async (models?: TtsModelDescriptor[]) => {
      const capability = capabilityRef.current;
      if (fakeServices || !capability || customSyncInFlightRef.current) return;
      const qwen = (models ?? rawTtsModelsRef.current).find(
        (item) => item.backendId === 'qwen3' && item.status === 'ready' && item.voiceCatalog,
      );
      ...
      try {
        for (const voice of missing) {
          try {
            await enrollStoredCustomVoice({ capability, voice });
          } catch {
            break;
          }
        }
      } finally {
        customSyncInFlightRef.current = false;
      }
    },
    [capabilityRef, customVoiceStoreRef, customVoicesRef],
  );
  const reconcileCurrentSettings = useCallback(
    (models: TtsModelDescriptor[], fallbackCatalog = voiceCatalogRef.current) => {
      const merged = mergeCustomModels(models);
      const fallback = withCustomVoices(fallbackCatalog, customVoicesRef.current);
      setSettingsModelState((previous) =>
        applyReconciled(
          previous,
          reconcileSettings(
            { selectedModel: previous.selectedModel, voice: previous.voice, voiceProfiles: previous.voiceProfiles },
            merged,
            fallback,
          ),
        ),
  ...
  const onCatalog = useCallback(
    (catalog: VoiceCatalog) => {
      ...
      if (ttsModelsRef.current.length === 0) {
        ...
        const rawFallback: TtsModelDescriptor = {
  ...
  const ensureCustomVoiceEnrolled = useCallback(
    async (voiceId: string) => {
      ...
      const qwen = rawTtsModelsRef.current.find(
        (item) => item.backendId === 'qwen3' && item.status === 'ready' && item.voiceCatalog,
      );
  ...
  const enrollVoice = useCallback(
    async (name: string, take: ReferenceTake) => {
      ...
      if (!(await store.save(record)))
        throw new Error('The local custom voice storage limit was reached. Delete a voice and try again.');
  ...
  const renameVoice = useCallback(
    async (voiceId: string, name: string) => {
      const store = customVoiceStoreRef.current;
    ...
    async (
      ...
      try {
        ...
        const next = applyReconciled(
          { agentName, persona, pi, selectedModel, voiceProfiles: profiles },
          { voice: activeVoice, selectedModel, voiceProfiles: profiles },
        );
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const next = applyReconciled(
          {
            agentName: stored?.agentName ?? DEFAULT_AGENT_NAME,
