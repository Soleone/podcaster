...
export function SettingsDialog({
  ...
  useEffect(() => {
    ...
    model.persona,
    model.pi?.model,
    model.pi?.thinkingLevel,
    model.selectedModel?.backendId,
    model.selectedModel?.modelId,
    model.voice.voiceId,
    model.voice.speedModifier,
    model.voice.tonePrompt,
  ...
  const piModelInvalid =
    !pi.model ||
    pi.model.startsWith('-') ||
    new TextEncoder().encode(pi.model).length > MAX_PI_MODEL_BYTES ||
    /\s/u.test(pi.model);
  const selectedDescriptor = models.find(
    (item) => item.backendId === selectedModel.backendId && item.modelId === selectedModel.modelId,
  );
  const selectedCatalog =
    selectedDescriptor?.voiceCatalog ??
    (selectedModel.backendId === DEFAULT_TTS_MODEL.backendId && selectedModel.modelId === DEFAULT_TTS_MODEL.modelId
      ? catalog
      : undefined);
  const displayCatalog =
    selectedModel.backendId === 'qwen3' ? withCustomVoices(selectedCatalog, customVoices) : selectedCatalog;
  ...
  const speedModifierInvalid =
    !Number.isFinite(speedModifierValue) ||
  ...
  const canSave =
    ...
    !personaInvalid &&
    !piModelInvalid &&
    !speedModifierInvalid &&
    !tonePromptInvalid &&
    !saving &&
    (!catalogReady || Boolean(voiceId));
  ...
  const togglePreview = async () => {
    ...
    try {
      const handle = await onPreviewVoice(
        voiceId,
        speedCapability.supported ? speedModifierValue : speedCapability.default,
        selectedModel,
        displayCatalog?.catalogId,
        selectedModel.backendId === 'qwen3' && tonePrompt.trim() ? tonePrompt.trim() : undefined,
        selectedModel.backendId === 'qwen3' ? language : undefined,
        request.signal,
      );
      ...
      void handle.finished.then(
        ...
        () => {
          if (previewHandleRef.current === handle) {
            previewHandleRef.current = undefined;
            setPreviewState('idle');
            setPreviewError('The preview stopped before it finished.');
          }
        },
      );
    } catch (caught) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className="h-[min(46rem,calc(100dvh-2rem))] max-h-[calc(100dvh-2rem)] min-h-0 grid-rows-[auto_minmax(0,1fr)_auto] overflow-hidden sm:max-w-lg"
        aria-describedby="settings-description"
      >
        <DialogHeader>
          <DialogTitle>Settings</DialogTitle>
          <DialogDescription id="settings-description">
            These apply to the next session you start. The active session is never changed mid-turn.
          </DialogDescription>
        </DialogHeader>
          ...
          <Tabs defaultValue="agent" className="min-h-0 min-w-0 flex-1">
            ...
            <TabsContent value="agent" className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto px-1 pt-4 pb-1">
              <FieldGroup>
                <Field data-invalid={agentNameInvalid || undefined}>
                  ...
                  />
                  <FieldDescription id="settings-agent-name-description">
                    Name shown above the assistant's messages in the conversation. It is kept out of the system prompt.
                  </FieldDescription>
                  {agentNameInvalid ? (
                    <FieldError>Agent name exceeds the {MAX_AGENT_NAME_BYTES}-byte limit.</FieldError>
                  ) : null}
                </Field>
              </FieldGroup>
              <section className="rounded-xl border p-4" aria-labelledby="settings-pi-service-heading">
                <h3 id="settings-pi-service-heading" className="text-sm font-medium">
                  Pi service
                </h3>
                <FieldGroup className="mt-3 gap-4">
                  ...
                  <Field>
                    ...
                    <FieldDescription>
                      Controls how much reasoning Pi uses before answering. The model may support only some levels.
                    </FieldDescription>
                  </Field>
                </FieldGroup>
              </section>
              <Accordion multiple defaultValue={['persona']} className="shrink-0 rounded-xl">
                <AccordionItem value="persona">
                  ...
                  <AccordionContent aria-labelledby="settings-persona-panel-label">
                    <span id="settings-persona-panel-label" className="sr-only">
                      Agent behavior settings
                    </span>
                    <Field data-invalid={personaInvalid || undefined}>
                      <FieldLabel className="sr-only" htmlFor="settings-persona">
                        Persona
                      </FieldLabel>
                      <InputGroup>
                        <InputGroupTextarea
                        ...
                        <InputGroupAddon align="block-end">
                          <InputGroupText
                            id="settings-persona-counter"
                            className={cn('text-xs', personaInvalid && 'text-destructive')}
                            aria-live="polite"
                          >
                      ...
                      <FieldDescription id="settings-persona-description">
                        Free-form instructions appended to the base system prompt when the next session starts. Empty is
                        allowed.
                      </FieldDescription>
                      {personaInvalid ? (
                        <FieldError>Persona exceeds the {MAX_PERSONA_BYTES / 1024} KiB limit.</FieldError>
                      ) : null}
                    </Field>
                  </AccordionContent>
                </AccordionItem>
                <AccordionItem value="system-prompt">
                  ...
                  <AccordionContent>
                    <Field data-disabled>
                      <FieldLabel className="sr-only" htmlFor="settings-system-prompt">
                        Base system prompt
                      </FieldLabel>
                      <Textarea
                      ...
                      />
                      <FieldDescription id="settings-system-prompt-description">
            ...
            <TabsContent value="voice" className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto px-1 pt-4 pb-1">
              {voiceNotice ? (
                <Alert
                  variant={
                    voiceNotice === 'missing_catalog' || voiceNotice === 'model_unavailable' ? 'destructive' : 'default'
                  }
                >
                  <AlertDescription>{VOICE_NOTICE_COPY[voiceNotice]}</AlertDescription>
                </Alert>
              ) : null}
              <FieldGroup>
                <Field>
                  ...
                  <FieldContent>
                    {models.length > 0 ? (
                      <Select value={ttsModelKey(selectedModel)} onValueChange={selectModel}>
                        <SelectTrigger id="settings-tts-model" className="w-full" aria-label="Speech model">
                          <SelectValue>
                            {selectedDescriptor?.label ?? `${selectedModel.backendId} · ${selectedModel.modelId}`}
                          </SelectValue>
                        </SelectTrigger>
                        <SelectContent>
                          <SelectGroup>
                            {models.map((item) => (
                              <SelectItem
                                key={ttsModelKey(item)}
                                value={ttsModelKey(item)}
                                disabled={item.status !== 'ready'}
                              >
                                {item.label}
                                {item.status !== 'ready' ? ' · unavailable' : ''}
                              </SelectItem>
                            ))}
                          </SelectGroup>
                        </SelectContent>
                      ...
                      <p className="text-sm text-muted-foreground">
                        The local audio engine will report selectable models when it is ready.
                      </p>
                    )}
                    {selectedDescriptor?.status === 'unavailable' ? (
                      <FieldDescription className="text-destructive">
                        {selectedDescriptor.reason ?? 'This model is unavailable.'} Kokoro remains the production
                        fallback.
                      </FieldDescription>
                    ) : selectedDescriptor ? (
                      <FieldDescription>
                        {selectedDescriptor.label} · backend {selectedDescriptor.backendId}
                      </FieldDescription>
                    ) : null}
                  </FieldContent>
                </Field>
                <Field>
                  ...
                  <FieldContent>
                      ...
                      <div className="min-w-0 flex-1">
                        {catalogReady ? (
                            ...
                            onValueChange={(value) => {
                              if (value) {
                                ...
                                setVoiceProfiles((previous) => ({
                                  ...previous,
                                  [ttsModelKey(selectedModel)]: {
                                    catalogId: displayCatalog!.catalogId,
                                    voiceId: value,
                                    speedModifier: speedCapability.supported
                                      ? speedModifierValue
                          ...
                          >
                            ...
                            <SelectContent>
                              <SelectGroup>
                                {displayCatalog!.voices.map((voice) => (
                          ...
                          <p className="text-sm text-muted-foreground">
                            Voice options appear once the selected model reports its verified catalog.
                          </p>
                        )}
                      </div>
                      {catalogReady && onPreviewVoice ? (
                        <Button
                          variant="outline"
                          size="icon"
                          className="shrink-0"
                    ...
                    {displayCatalog ? (
                      <FieldDescription>
                        Backend {displayCatalog.backendId} · model {displayCatalog.modelId} · revision{' '}
                        {displayCatalog.revision.slice(0, 8)}
                      </FieldDescription>
                    ) : null}
                  </FieldContent>
                </Field>
                {selectedModel.backendId === 'qwen3' ? (
                  <Field data-invalid={tonePromptInvalid || undefined}>
                    ...
                    <FieldContent>
                      <Textarea
                        ...
                        value={tonePrompt}
                        onChange={(event) => {
                          setTonePrompt(event.target.value);
                          setVoiceProfiles((previous) => ({
                            ...previous,
                            [ttsModelKey(selectedModel)]: {
                              catalogId: displayCatalog?.catalogId ?? '',
                      ...
                      <div
                        ...
                        id="settings-voice-tone-counter"
                        aria-live="polite"
                      >
                      ...
                      {tonePromptInvalid ? (
                        <FieldError>Tone instruction exceeds the {MAX_VOICE_TONE_PROMPT_BYTES}-byte limit.</FieldError>
                      ) : null}
                    </FieldContent>
                  </Field>
                ) : null}
                  ...
                  <Field>
                    ...
                    <FieldContent>
                      ...
                      >
                        ...
                        <SelectContent>
                          <SelectGroup>
                            {QWEN_VOICE_LANGUAGES.map((option) => (
                      ...
                      <FieldDescription>
                        Qwen supports these ten synthesis languages. The selected language applies to the next session
                        and voice preview.
                      </FieldDescription>
                    </FieldContent>
                  </Field>
                ...
                <Field data-invalid={speedModifierInvalid || undefined}>
                  ...
                  <FieldContent>
                    <Input
                      ...
                      onChange={(event) => {
                        setSpeedModifier(event.target.value);
                        setVoiceProfiles((previous) => ({
                          ...previous,
                          [ttsModelKey(selectedModel)]: {
                            catalogId: displayCatalog?.catalogId ?? '',
                    ...
                    />
                    <FieldDescription id="settings-voice-speed-description">
                      {speedCapability.supported
                        ? `Use this model's declared range ${speedCapability.min} to ${speedCapability.max}. ${speedCapability.default} is its normal speed.`
                        : 'This model does not declare playback-speed control; its normal speed is fixed at 1.0.'}
                    </FieldDescription>
                    {speedModifierInvalid ? (
                      <FieldError>Speed modifier is outside this model's declared capability.</FieldError>
                    ) : null}
                  </FieldContent>
                </Field>
              </FieldGroup>
