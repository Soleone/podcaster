...
export function settingsDigest(settings: { agentName: string; persona: string; voice: VoicePreference; selectedModel?: TtsModelSelection }): string {
  ...
  const source = `${settings.agentName}\u0000${settings.persona}\u0000${model.backendId}\u0000${model.modelId}\u0000${settings.voice.catalogId}\u0000${settings.voice.voiceId}\u0000${settings.voice.speedModifier}`;
...
export function reconcileVoice(preference: VoicePreference | undefined, catalog: VoiceCatalog | undefined): { voice: VoicePreference; notice?: VoiceNoticeReason } {
  ...
  if (preference && isValidVoicePreference(preference) && isVoiceInCatalog(catalog, preference.voiceId)) {
    return { voice: { catalogId: catalog.catalogId, voiceId: preference.voiceId, speedModifier: speed }, notice: speedNotice ?? 'rebase' };
  }
  return { voice: { catalogId: catalog.catalogId, voiceId: catalog.defaultVoiceId, speedModifier: speed }, notice: speedNotice ?? 'defaulted' };
}
