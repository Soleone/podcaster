...
import {
  ...
  MAX_CUSTOM_VOICES,
  MIN_CUSTOM_VOICE_MS,
  VOICE_ENROLLMENT_CONSENT_ACK,
  VOICE_ENROLLMENT_CONSENT_COPY,
  VOICE_ENROLLMENT_RETENTION_COPY,
...
import {
  ReferenceRecorder,
  ReferenceRecordingError,
  finalizeReferenceRecording,
  referenceErrorCopy,
  type MicrophoneState,
  type ReferenceTake,
} from '../voice-enrollment/recorder';
...
export function CustomVoiceSection({ voices, onEnroll, onDelete, onRename }: CustomVoiceSectionProps) {
  ...
  const start = async () => {
    if (!consent) {
      setError('Check the consent box before requesting the microphone.');
      return;
    }
    setError(undefined);
  ...
  const save = async () => {
    ...
    if (!normalized) {
      setError('Give the voice a name before saving.');
      return;
    }
    setBusy(true);
    setError(undefined);
  ...
  const rename = async (voice: CustomVoiceRecord) => {
    ...
    if (!normalized) {
      setError('Voice names cannot be empty.');
      return;
    }
    setBusy(true);
    try {
      await onRename(voice.voiceId, normalized);
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : 'The voice could not be renamed.');
    } finally {
      setBusy(false);
    }
  };
  ...
  const remove = async (voice: CustomVoiceRecord) => {
    setBusy(true);
    try {
      await onDelete(voice.voiceId);
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : 'The voice could not be deleted.');
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <section className="rounded-xl border p-3" aria-labelledby="custom-voice-heading">
      <div className="mb-3">
        <h3 id="custom-voice-heading" className="text-sm font-medium">
          Voice cloning
        </h3>
      ...
      <label className="flex items-start gap-2 text-sm">
        <input
          ...
          aria-describedby="custom-voice-consent-copy"
        />
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        {!recording ? (
          <>
            <Button type="button" variant="outline" onClick={() => void start()} disabled={!consent || busy}>
        ...
        {recording ? (
          <span className="text-sm tabular-nums" role="status">
            {(elapsedMs / 1000).toFixed(1)}s / {MAX_CUSTOM_VOICE_MS / 1000}s
          </span>
        ) : null}
      </div>
      <p className="mt-2 text-xs text-muted-foreground">
        Use a clear {MIN_CUSTOM_VOICE_MS / 1000} to {MAX_CUSTOM_VOICE_MS / 1000} second sample in a quiet room.
      </p>
      ...
      {microphoneState === 'busy' ? (
        <p className="mt-1 text-xs text-destructive" role="status">
          The microphone is busy. Close other recording apps and try again.
        </p>
      ) : null}
      {take ? (
        <div className="mt-3 rounded-lg bg-muted/50 p-3">
          ...
          <Button type="button" className="mt-3" onClick={() => void save()} disabled={busy || !consent}>
            {busy ? (
              <>
      ...
      {voices.length > 0 ? (
        <div className="mt-4 space-y-2" aria-label="Saved custom voices">
          <p className="text-xs font-medium">
            Saved on this device ({voices.length}/{MAX_CUSTOM_VOICES})
          </p>
          {voices.map((voice) => (
            <div
              key={voice.voiceId}
