...
const qwenModel: TtsModelDescriptor = {
  ...qwenCatalog,
...
describe('settings-model voice reconciliation', () => {
  ...
  it('honors exactOptionalPropertyTypes on the model', () => {
    const withNotice: SettingsModel = applyReconciled(
      { agentName: 'Ada', persona: 'persona' },
      reconcileVoice({ catalogId: 'c1', voiceId: 'af_bella', speedModifier: 1.0 }, { ...catalog, catalogId: 'c2' }),
    );
    expect(withNotice.agentName).toBe('Ada');
    expect(withNotice.notice).toBe('rebase');
    const clean: SettingsModel = applyReconciled(
      { agentName: 'Ada', persona: 'persona' },
      reconcileVoice({ catalogId: 'c1', voiceId: 'af_bella', speedModifier: 1.0 }, catalog),
    );
  ...
  it('keeps independent Kokoro and Qwen profiles while switching models', () => {
    ...
    const backOnKokoro = reconcileSettings({ selectedModel: kokoro, voiceProfiles: onQwen.voiceProfiles }, [
      kokoroModel,
      qwenModel,
    ]);
  ...
  it('falls back to Kokoro when the saved Qwen model is unavailable', () => {
    ...
    const unavailable: TtsModelDescriptor = {
      ...qwenUnavailableBase,
      status: 'unavailable',
      reason: 'CUDA unavailable',
      fallback: { backendId: 'kokoro', modelId: 'kokoro-82m-onnx' },
    };
    const result = reconcileSettings(
      { selectedModel: { backendId: 'qwen3', modelId: 'qwen3-tts-0.6b' }, voiceProfiles: {} },
      [kokoroModel, unavailable],
    );
  ...
  it('produces a stable, input-sensitive digest', () => {
    const a = settingsDigest({
      agentName: 'Ada',
      persona: 'x',
      voice: { catalogId: 'c', voiceId: 'v', speedModifier: 1.0 },
    });
    expect(a).toBe(
      settingsDigest({ agentName: 'Ada', persona: 'x', voice: { catalogId: 'c', voiceId: 'v', speedModifier: 1.0 } }),
    );
    expect(a).not.toBe(
      settingsDigest({ agentName: 'Lin', persona: 'x', voice: { catalogId: 'c', voiceId: 'v', speedModifier: 1.0 } }),
    );
    expect(a).not.toBe(
      settingsDigest({ agentName: 'Ada', persona: 'y', voice: { catalogId: 'c', voiceId: 'v', speedModifier: 1.0 } }),
    );
    expect(a).not.toBe(
      settingsDigest({ agentName: 'Ada', persona: 'x', voice: { catalogId: 'c', voiceId: 'w', speedModifier: 1.0 } }),
    );
  });
});
