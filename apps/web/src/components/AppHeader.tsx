...
export function AppHeader({
  ...
  return (
    <header className="sticky top-0 z-10 bg-background py-3" data-slot="app-header">
      <div className="mx-auto flex min-h-12 w-[min(56rem,calc(100%_-_2rem))] items-center justify-between gap-2 rounded-full border border-primary/20 bg-primary-foreground px-2 py-1.5 shadow-sm">
        ...
        <div className="flex items-center gap-1">
          <ServiceStatusPopover
            ...
            piSettings={piSettings}
            onRefresh={onRefreshServiceStatus}
            refreshing={refreshingServiceStatus}
            privacyAcknowledged={privacyAcknowledged}
            connected={Boolean(capability)}
            onConnect={onConnectServices}
