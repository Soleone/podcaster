...
export function SessionIndex(props: SessionIndexProps) {
  ...
  const buildExport = useCallback(
    (sessionId: string) => (onProgress?: ExportOnProgress) =>
      exportSessionRecording(sessionId, props.writer, onProgress),
    [props.writer],
  );
  ...
  return (
    <main className="mx-auto mt-5 mb-8 w-[min(56rem,calc(100%_-_2rem))]">
      <header className="mb-5 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Start with a draft. Services only matter when you go live.
          </p>
        </div>
        <Button className="min-h-11" onClick={() => void props.onCreateDraft()} disabled={props.creatingDraft}>
          {props.creatingDraft ? (
            <>
      ...
      {props.liveSessionId ? (
        <Card aria-label={props.liveSessionPaused ? 'Paused session' : 'Active session'} className="mb-8">
          <CardContent className="flex flex-col gap-4 sm:flex-row sm:items-center">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">
                {props.liveSessionPaused ? 'Paused session' : 'Active session'}
              </p>
              <p className="mt-1 font-medium tabular-nums">Session {shortSessionId(props.liveSessionId)}</p>
              <p className="mt-1 flex items-center gap-1.5 text-sm text-muted-foreground">
                <Clock className="size-3.5 shrink-0" aria-hidden="true" />
                {props.liveSessionPaused ? 'Active time' : 'Running for'} {formatDuration(props.elapsedSeconds)}
              </p>
            </div>
            <Link
              to={`/session/${props.liveSessionId}`}
      ...
      <section aria-labelledby="session-list-title">
        <h2 id="session-list-title" className="mb-3 flex items-center gap-2 text-base font-medium leading-snug">
          ...
          <Card>
            <CardContent className="flex items-center gap-2 text-sm text-muted-foreground">
              {summaries === undefined ? (
                ...
                'No sessions yet. Create one above and it will appear here.'
              )}
            </CardContent>
          </Card>
        ) : null}
          ...
          <ul className="flex list-none flex-col gap-3 p-0">
            {rows.map((row) => (
              <SessionRow
                ...
                row={row}
                live={row.session.sessionId === props.liveSessionId}
                buildExport={buildExport}
                onContinue={() => props.onContinueSession(row.session.sessionId)}
              />
            ))}
          </ul>
        ) : null}
...
function SessionRow(props: {
  row: SessionSummary;
  live: boolean;
  buildExport: (sessionId: string) => (onProgress?: ExportOnProgress) => Promise<Blob>;
  onContinue: () => void;
}) {
  ...
  const recordingLabel = !recordingEnabled
    ? 'Recording off'
    : recordingItemCount === 0
      ? 'No recording'
      : `${recordingItemCount} message${recordingItemCount === 1 ? '' : 's'} recorded`;
  return (
    <li>
      <Card size="sm">
        <CardContent className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0 flex-1">
            <p className="flex items-center gap-2">
              ...
              <Badge variant={props.live || (!stopped && !paused && !draft) ? 'default' : 'secondary'}>
                {props.live
                  ? paused
                    ? 'Paused'
                    : 'Active'
            ...
            {preview ? (
              <p className={cn('mt-1 truncate text-sm', session.title && 'text-muted-foreground')}>{preview}</p>
            ) : null}
            <p className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
              <span>
                {draft ? 'Created' : 'Started'} {formatWhen(session.startedAt)}
              </span>
          ...
          <div className="flex w-full flex-wrap items-center justify-end gap-2 sm:w-auto">
            ...
            <ExportPopover
              ...
              buildExport={props.buildExport(session.sessionId)}
              disabled={recordingItemCount === 0}
              variant="outline"
              size="sm"
            />
          </div>
