...
describe('session archive', () => {
  it('lists sessions with turn and recording counts, newest first', async () => {
    ...
    await writer.apply({
      ...
      payload: { turnId: 't1', text: 'first', endpointComplete: true },
    });
    await writer.apply({
      protocolVersion: 1,
      eventId: 'e2',
      ...
      monotonicMs: 2,
      type: 'transcript.final',
      payload: { turnId: 't2', text: 'second', endpointComplete: true },
    });
    await writer.apply({
      protocolVersion: 1,
      eventId: 'e3',
      ...
      monotonicMs: 3,
      type: 'transcript.final',
      payload: { turnId: 't3', text: 'third', endpointComplete: true },
    });
  ...
  it('rebuilds a stopped conversation with stored playback dispositions', async () => {
    ...
    await writer.apply({
      ...
      monotonicMs: 1,
      type: 'transcript.final',
      payload: { turnId: 't1', text: 'a question', endpointComplete: true },
    });
    await writer.apply({
      protocolVersion: 1,
      eventId: 'e2',
      ...
      payload: { turnId: 't1', responseId: 'r1', posture: 'question', text: 'an answer' },
    });
    await writer.apply({
      protocolVersion: 1,
      eventId: 'e3',
      ...
      monotonicMs: 3,
      type: 'tts.started',
      payload: { responseId: 'r1', playbackId: 'p1', sampleRate: 24000 },
    });
    await writer.apply({
      protocolVersion: 1,
      eventId: 'e4',
      ...
      payload: { playbackId: 'p1', cancelledEpoch: 0, finalPlayedSampleOffset: 100, reason: 'completed' },
    });
    ...
    expect(view.conversationItems.find((item) => item.kind === 'user')).toMatchObject({
      kind: 'user',
      text: 'a question',
      status: 'stable',
    });
  ...
  it('rebuilds a paused conversation without making an unfinished response resumable', async () => {
    ...
    expect(view.conversationItems.find((item) => item.kind === 'assistant')).toMatchObject({
