...
export function DraftSession(props: DraftSessionProps) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setSaving(true);
      void props.writer
        .updateDraftSession(props.sessionId, preparation, title)
        .then((result) => {
  ...
  const enableMicrophone = async () => {
    ...
    try {
      await props.onEnableMicrophone();
    } catch (cause) {
      setMicrophoneError(cause instanceof Error ? cause.message : 'Microphone access was not granted.');
    }
  };
  ...
  return (
    <main className="mx-auto mt-5 mb-8 w-[min(56rem,calc(100%_-_2rem))]">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <Button variant="ghost" size="sm" className="-ml-2 mb-3 text-muted-foreground" onClick={props.onBack}>
            <ArrowLeft data-icon="inline-start" aria-hidden="true" />
            All sessions
          </Button>
        ...
        <CardContent className="flex flex-col gap-4">
          <label className="flex cursor-pointer items-start gap-3 text-sm">
            <input
              ...
              onChange={(event) => setPreparation((value) => ({ ...value, enabled: event.target.checked }))}
              disabled={starting}
            />
            <span>
          ...
          {preparation.enabled ? (
            <div className="grid gap-4 border-t pt-4">
              <label className="grid gap-1.5 text-sm">
                <span className="font-medium">Rough topic</span>
                <textarea
                  className={cn(
                    'min-h-24 resize-y rounded-md border bg-background px-3 py-2 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring',
                    topicInvalid && 'border-destructive',
                  )}
                  value={preparation.topic}
                  onChange={(event) => setPreparation((value) => ({ ...value, topic: event.target.value }))}
                  maxLength={2048}
                ...
                <span className="text-xs text-muted-foreground">
                  Keep it brief. This is sent to the configured research provider.
                </span>
              </label>
              <label className="grid gap-1.5 text-sm">
                <span className="font-medium">Preparation depth</span>
                <select
      ...
      <Card className={cn('mt-4', serviceWaiting && 'border-amber-500/30')}>
        <CardContent className="flex flex-col gap-3 py-4">
          ...
          {!props.privacyAcknowledged || !props.capability ? (
            <Button variant="outline" className="w-full sm:w-fit" onClick={props.onConnectServices}>
              {props.privacyAcknowledged ? 'Reconnect services' : 'Review privacy &amp; connect'}
            </Button>
          ) : null}
          {props.privacyAcknowledged && !props.microphoneGranted ? (
            <Button
              variant="outline"
        ...
        <CardFooter className="justify-end border-t">
          <Button className="min-h-11 w-full sm:w-auto" onClick={() => void start()} disabled={!canStart}>
            {starting ? (
              <>
