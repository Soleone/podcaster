...
afterEach(async () => {
  for (const name of databases.splice(0)) await new Promise<void>(resolve => { const request = indexedDB.deleteDatabase(name); request.onsuccess = request.onerror = request.onblocked = () => resolve(); });
});
...
async function setup() {
  ...
  const runtime = await createLiveSessionRuntime({
    sessionId: 'session', capability: 'fake', writer, transport, fake: true,
    openRecordingStore: async () => { calls.push('recording.open'); return store; },
    createCapture: async () => { calls.push('capture.start'); return { stop: async () => { captureStops++; calls.push('capture.stop'); } }; },
    createEncoder: () => async () => new Uint8Array(),
    initialState: { ...initialSessionState, dominant: 'listening', announcement: 'Listening' },
    seed: 'seed', reasoningMode: 'full', settings: { version: 1, persona: '', voice: { catalogId: 'catalog', voiceId: 'voice', speedModifier: 1 } },
    activate: async () => { calls.push('activate'); const result = await writer.beginSession({ sessionId: 'session', sessionSeed: 'seed', personaDigest: 'digest' }); if (!result.ok) throw new Error(result.degradedReason); },
    callbacks: { onView: () => undefined, onTransportFailure: () => undefined, onRecordingChanged: () => undefined },
  });
  return { runtime, writer, transport, calls, get captureStops() { return captureStops; } };
}
...
describe('live session runtime', () => {
  ...
  it('rolls back a failed composition and makes dispose idempotent', async () => {
    ...
    const store = await RecordingStore.open(indexedDB, `${name}-recordings`);
    let stops = 0;
    await expect(createLiveSessionRuntime({
      sessionId: 'failed', capability: 'fake', writer, transport, fake: true,
      openRecordingStore: async () => store,
      createCapture: async () => ({ stop: async () => { stops++; } }),
      createEncoder: () => async () => new Uint8Array(),
      seed: 'seed', reasoningMode: 'full', settings: { version: 1, persona: '', voice: { catalogId: 'catalog', voiceId: 'voice', speedModifier: 1 } },
      activate: async () => { const result = await writer.beginSession({ sessionId: 'failed', sessionSeed: 'seed', personaDigest: 'digest' }); if (!result.ok) throw new Error(result.degradedReason); throw new Error('start failed'); },
      callbacks: { onView: () => undefined, onTransportFailure: () => undefined, onRecordingChanged: () => undefined },
