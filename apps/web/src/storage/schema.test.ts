...
describe('IndexedDB schema', () => {
  it('migrates a version-1 turn without losing stable conversation data', async () => {
    ...
    const legacy = await new Promise<IDBDatabase>((resolve, reject) => {
      ...
      request.onupgradeneeded = () => {
        ...
        store.createIndex('responseId', 'responseId');
        store.createIndex('playbackId', 'playbackId');
        store.put({
          key: 's:t',
          sessionId: 's',
          turnId: 't',
          stableText: 'kept words',
          interrupted: false,
    ...
    ).toMatchObject({ stableText: 'kept words', timelineSequence: 1, continuationState: 'none', controlOnly: false });
    db.close();
  });
  ...
  it('migrates chronology by creation time rather than conflicting object-store key order', async () => {
    ...
    expect(rows.sort((a, b) => a.timelineSequence - b.timelineSequence).map((row) => row.turnId)).toEqual([
      'earlier',
      'later',
      'tie-first',
      'tie-second',
  ...
  it('migrates a version-2 database to the current version with the recordingItems store and indexes', async () => {
    ...
    expect(Array.from(recordingItems.indexNames)).toEqual(
      expect.arrayContaining(['sessionId', 'turnId', 'playbackId', 'recordSeq']),
    );
    const kept = db.transaction(STORES.turns, 'readonly').objectStore(STORES.turns).get('kept');
    expect(
      await new Promise((resolve) => {
        kept.onsuccess = () => resolve(kept.result);
      }),
    ).toMatchObject({ stableText: 'kept words' });
    db.close();
  });
    ...
    const legacy = await new Promise<IDBDatabase>((resolve, reject) => {
      ...
      request.onupgradeneeded = () => {
        ...
        recordingItems.put({
          ...
          createdAt: '2026-01-01T00:00:00Z',
          monotonicMs: 0,
          data: new Blob([new Uint8Array([1, 2, 3, 4, 5])], { type: 'audio/mpeg' }),
        });
      };
      request.onsuccess = () => resolve(request.result);
      request.onerror = () => reject(request.error);
