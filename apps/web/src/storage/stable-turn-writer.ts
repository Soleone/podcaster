...
interface PlaybackAccounting {
  ...
  terminal: null | {
    ...
    reason: 'completed' | 'cancelled' | 'stopped' | 'failed';
    eventId: string;
  };
}
...
export function sessionActiveDurationMs(session: StoredSession, atMs = Date.now()): number {
  ...
  const ended =
    session.state === 'active' ? atMs : (timestampMs(session.endedAt) ?? timestampMs(session.updatedAt) ?? atMs);
...
function preparationDraft(value: SessionPreparationDraft): SessionPreparationDraft {
  ...
  if (new TextEncoder().encode(topic).length > MAX_PLANNING_TOPIC_BYTES)
    throw new DraftRejected('The preparation topic is too long.');
  if (!['light', 'standard', 'deep'].includes(value.depth))
    throw new DraftRejected('The preparation depth is invalid.');
...
export class StableTurnWriter {
  ...
  async createDraftSession(input: {
    ...
    return this.guard(async () => {
      ...
      if (existing) {
        transaction.abort();
        throw new DraftRejected('The session already exists.');
      }
  ...
  async updateDraftSession(
    ...
    return this.guard(async () => {
      ...
      if (!session || session.state !== 'draft') {
        transaction.abort();
        throw new DraftRejected('Only a not-started session can be edited.');
      }
  ...
  async beginSession(input: {
    ...
    planning?: StoredSession['planning'];
    startedAt?: string;
  }): Promise<StorageResult> {
    return this.guard(async () => {
  ...
  async pauseSession(
    sessionId: string,
    pausedAt = isoNow(),
    playbacks: readonly PlaybackPauseCheckpoint[] = [],
  ): Promise<StorageResult> {
    return this.guard(async () => {
      ...
      if (!session) {
        transaction.abort();
        throw new PauseRejected('The session could not be found.');
      }
      if (session.state === 'draft') {
        transaction.abort();
        throw new PauseRejected('The session has not started yet.');
      }
      ...
      for (const turn of turns) {
        const checkpoint = playbacks.find(
          (candidate) =>
            candidate.responseId === turn.responseId &&
            candidate.playbackId === turn.playbackId &&
            candidate.outputEpoch === turn.outputEpoch,
        );
        let changed = false;
        if (checkpoint) {
          ...
          if (checkpoint.generatedSamples > 0)
            turn.generatedSamples = Math.max(turn.generatedSamples, checkpoint.generatedSamples);
          turn.deliveredSampleOffset =
            turn.generatedSamples > 0
              ? Math.min(turn.pendingDeliveredOffset, turn.generatedSamples)
              : turn.deliveredSampleOffset;
          changed = true;
        }
        ...
        if (
          (turn.assistantText !== null || responseInFlight || turn.playbackId !== null || checkpoint) &&
      ...
      store.put({
        ...
        endedAt: null,
        updatedAt: pausedAt,
        activeDurationMs: sessionActiveDurationMs(session, atMs),
        runningSince: null,
        pausedAt,
      });
  ...
  async apply(event: StableEvent): Promise<StorageResult> {
    ...
    return this.guard(async () => {
      ...
      if (event.type === 'session.state') {
        ...
        if (session && planning) {
          ...
          const identityMatches =
            (!prior?.topic || !planning.topic || prior.topic === planning.topic) &&
          ...
          if (identityMatches && notStale)
            sessions.put({ ...session, planning: { ...prior, ...planning }, updatedAt: at });
        }
      }
      ...
      if (event.type === 'transcript.final' && turn) {
        ...
        if (
          !accounting.terminal &&
          (reason === 'completed' || reason === 'cancelled' || reason === 'stopped' || reason === 'failed')
        ) {
        ...
        if (
          disposition === 'resume_noise' ||
          disposition === 'resume_fragment' ||
          disposition === 'resume_requested' ||
          disposition === 'accept_takeover'
        )
          turn.interruptionDisposition = disposition;
        ...
        if (interrupted && interrupted.key !== turn.key) {
          interrupted.pausedSampleOffset =
            numberValue(event.payload.pausedSampleOffset) ?? interrupted.pausedSampleOffset;
  ...
  private mergeDelivered(turn: StoredTurn, offset: number): void {
    ...
    if (turn.generatedSamples > 0)
      turn.deliveredSampleOffset = Math.max(
        turn.deliveredSampleOffset,
        Math.min(turn.pendingDeliveredOffset, turn.generatedSamples),
      );
  }
  ...
  private async guard(operation: () => Promise<void>): Promise<StorageResult> {
    try {
      ...
      if (error instanceof PauseRejected || error instanceof DraftRejected)
        return { ok: false, degradedReason: error.message };
      ...
      return {
        ...
        degradedReason:
          name === 'QuotaExceededError'
            ? 'Local storage is full. Earlier stable turns are preserved.'
            : 'Stable local storage is unavailable. Earlier saved work was not changed.',
      };
    }
  }
}
