...
let sequence = 0;
const open = async () => {
  const name = `podcaster-test-${++sequence}`;
  databases.push(name);
...
describe('StableTurnWriter', () => {
  it('recovers the same active session seed after refresh', async () => {
    ...
    expect(await reopened.recoverActiveSession()).toMatchObject({
      sessionId: 'session',
      sessionSeed: 'seed-1',
      state: 'active',
    });
  ...
  it('creates an editable draft without claiming an active session', async () => {
    ...
    expect(await writer.getSession('draft')).toMatchObject({
      state: 'draft',
      preparation: { enabled: true, topic: 'Local radio', depth: 'deep' },
    });
    await writer.beginSession({ sessionId: 'draft', sessionSeed: 'ignored', personaDigest: 'live-digest' });
    expect(await writer.getSession('draft')).toMatchObject({
      state: 'active',
      sessionSeed: 'seed-draft',
      personaDigest: 'live-digest',
    });
    writer.close();
  });
  ...
  it('persists and carries a draft title through the session lifecycle', async () => {
    const { writer } = await open();
    await writer.createDraftSession({ sessionId: 'draft', sessionSeed: 'seed-draft' });
    await writer.updateDraftSession(
      'draft',
      { enabled: false, topic: '', depth: 'standard' },
  ...
  it('persists planning lifecycle and keeps the first topic/depth/notes on reconnect', async () => {
    ...
    await writer.apply(
      event('session', 'session.state', {
        phase: 'planning',
        personaDigest: 'digest',
        planning: { ...planning, progress: 55, detail: 'Researching' },
      }),
    );
    await writer.apply(
      event('session', 'session.state', {
        phase: 'ready',
        personaDigest: 'digest',
        planning: { ...planning, status: 'ready', progress: 100, notes: 'Useful facts\nTalking points' },
      }),
    );
    await writer.apply(
      event('session', 'session.state', {
    ...
    await writer.apply(
      event('session', 'session.state', {
        phase: 'ready',
        personaDigest: 'digest',
        planning: { status: 'failed', topic: 'Changed topic', depth: 'deep', progress: 100 },
      }),
    );
    expect(await writer.getSession('session')).toMatchObject({
      planning: {
        status: 'ready',
        topic: 'The future of local radio',
        depth: 'standard',
    ...
    await writer.beginSession({
      ...
      personaDigest: 'new-digest',
      planning: { status: 'planning', topic: 'Changed topic', depth: 'deep' },
    });
    expect(await writer.getSession('session')).toMatchObject({
      sessionSeed: 'seed',
      personaDigest: 'digest',
      planning: { topic: 'The future of local radio', depth: 'standard', notes: 'Useful facts\nTalking points' },
    });
    writer.close();
  });
  ...
  it('checkpoints a paused session, interrupts unfinished playback, and preserves frozen identity on resume', async () => {
    ...
    await writer.apply(
      event('session', 'transcript.final', { turnId: 'turn', text: 'question', endpointComplete: true }),
    );
    await writer.apply(
      event('session', 'reasoning.final', {
        turnId: 'turn',
        responseId: 'response',
        posture: 'question',
        text: 'answer',
    ...
    await writer.apply(
      event('session', 'tts.started', { responseId: 'response', playbackId: 'playback', sampleRate: 24000 }),
    );
    expect(
      (
        await writer.pauseSession('session', '2026-01-01T00:01:30.000Z', [
          {
            responseId: 'response',
            playbackId: 'playback',
            outputEpoch: 0,
            pausedSampleOffset: 240,
            generatedSamples: 1_000,
          },
        ])
      ).ok,
    ).toBe(true);
    ...
    await writer.beginSession({
      sessionId: 'session',
      sessionSeed: 'new-seed-is-ignored',
      personaDigest: 'new-digest-is-ignored',
      settings: { ...settings, persona: 'changed' },
    });
    expect(await writer.getSession('session')).toMatchObject({
      state: 'active',
      sessionSeed: 'seed-1',
  ...
  it('marks a response that is still reasoning as interrupted on pause', async () => {
    ...
    await writer.apply(
      event('session', 'reasoning.started', { turnId: 'turn', responseId: 'response', posture: 'question' }),
    );
  ...
  it('keeps generated text separate and resolves reordered delivery with first-terminal wins', async () => {
    ...
    await writer.apply(
      event('s', 'reasoning.final', { turnId: 't', responseId: 'r', posture: 'question', text: 'response text' }),
    );
    await writer.apply(
      event('s', 'playback.progress', {
        playbackId: 'p',
        outputEpoch: 0,
        playedSampleOffset: 900,
        generatedSamples: 1000,
      }),
    );
    await writer.apply(
      event('s', 'playback.stopped', {
        playbackId: 'p',
        cancelledEpoch: 0,
        finalPlayedSampleOffset: 700,
        reason: 'cancelled',
      }),
    );
    ...
    expect(turn).toMatchObject({
      ...
      generatedSamples: 800,
      deliveredSampleOffset: 700,
  ...
  it('does not mark a rejected provisional interruption as delivered history', async () => {
    ...
    await writer.apply(event('s', 'transcript.final', { turnId: 't', text: 'hello' }));
    await writer.apply(
      event('s', 'reasoning.final', { turnId: 't', responseId: 'r', posture: 'question', text: 'answer' }),
    );
    await writer.apply(event('s', 'tts.started', { responseId: 'r', playbackId: 'p', sampleRate: 24000 }));
  ...
  it('persists pause identity and a control-only resume without interrupting the original response', async () => {
    ...
    await writer.apply(
      event('s', 'reasoning.final', { turnId: 'original', responseId: 'r', posture: 'question', text: 'answer' }),
    );
    await writer.apply(event('s', 'tts.started', { responseId: 'r', playbackId: 'p', sampleRate: 24000 }));
    await writer.apply(
      event('s', 'playback.paused', {
        responseId: 'r',
        playbackId: 'p',
        outputEpoch: 0,
        pausedSampleOffset: 240,
  ...
  it('reconciles early tts.started through reasoning.started identity and upserts final without losing playback', async () => {
    ...
    await writer.apply(
      event('s', 'reasoning.final', { turnId: 't', responseId: 'r', posture: 'question', text: 'answer text' }),
    );
    expect(await writer.getTurns('s')).toMatchObject([
      { turnId: 't', responseId: 'r', playbackId: 'p', assistantText: 'answer text', generatedSamples: 960 },
    ]);
    writer.close();
  });
  ...
  it('records a response failure on the matching turn instead of session scope', async () => {
    ...
    expect(await writer.getTurns('s')).toMatchObject([
      { turnId: 't', responseId: 'r', failures: ['tts_failed'], interrupted: true, assistantText: null },
    ]);
    writer.close();
  });
  ...
  it('isolates reused playback IDs by session and epoch', async () => {
    ...
    for (const [sessionId, turnId, responseId] of [
      ...
      await writer.apply(
        event(sessionId, 'reasoning.final', { turnId, responseId, posture: 'question', text: 'answer' }),
      );
    ...
    await writer.apply(
      event('s1', 'reasoning.final', { turnId: 't3', responseId: 'r3', posture: 'question', text: 'later answer' }, 1),
    );
    ...
    await writer.apply(
      event('s1', 'playback.stopped', {
        playbackId: 'shared',
        cancelledEpoch: 0,
        finalPlayedSampleOffset: 200,
        reason: 'cancelled',
      }),
    );
    await writer.apply(
      event('s2', 'playback.stopped', {
        playbackId: 'shared',
        cancelledEpoch: 0,
        finalPlayedSampleOffset: 800,
        reason: 'completed',
    ...
    await writer.apply(
      event(
        's1',
        'playback.stopped',
        { playbackId: 'shared', cancelledEpoch: 1, finalPlayedSampleOffset: 600, reason: 'completed' },
        1,
      ),
    );
  ...
  it('reports quota degradation without changing an earlier stable turn', async () => {
    ...
    expect(result).toMatchObject({
      ok: false,
      degradedReason: expect.stringContaining('Earlier stable turns are preserved'),
    });
