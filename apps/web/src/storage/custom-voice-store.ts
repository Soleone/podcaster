...
import { openPodcasterDatabase, requestResult, STORES, transactionDone, type DatabaseFactory, type StoredCustomVoice } from './schema';
...
function validRecord(value: unknown): value is CustomVoiceRecord {
  ...
  return isValidCustomVoiceId(row.voiceId)
    && typeof row.name === 'string' && row.name.trim().length > 0 && utf8Bytes(row.name) <= MAX_VOICE_NAME_BYTES
    && typeof row.refSha256 === 'string' && /^[a-f0-9]{64}$/.test(row.refSha256)
    && row.voiceId === `custom:${row.refSha256.slice(0, 24)}`
    && row.sampleRate === CUSTOM_VOICE_SAMPLE_RATE
    && typeof row.durationMs === 'number' && Number.isInteger(row.durationMs) && row.durationMs >= 3_000 && row.durationMs <= 20_000
    && row.wav instanceof Blob && typeof row.byteLength === 'number' && row.byteLength === row.wav.size && isReferenceSizeValid(row.byteLength)
...
export class CustomVoiceStore {
  ...
  async save(record: CustomVoiceRecord): Promise<boolean> {
    ...
    try {
      ...
      const total = valid.reduce((sum, item) => sum + item.byteLength, 0) - (existing?.byteLength ?? 0) + record.byteLength;
      if (!existing && valid.length >= MAX_CUSTOM_VOICES) { transaction.abort(); return false; }
      if (total > MAX_CUSTOM_VOICE_TOTAL_BYTES) { transaction.abort(); return false; }
      store.put(record);
