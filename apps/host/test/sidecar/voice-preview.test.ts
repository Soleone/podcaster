...
interface RecordedRequest { type: string; payload: Record<string, unknown> }
  ...
  wss.on('connection', (socket, request) => {
    ...
    socket.on('message', raw => {
      ...
      if (message.type === 'stream.open') {
        if (options.busy) {
          socket.send(JSON.stringify({ type: 'sidecar.failure', payload: { code: 'invalid_message', recoverable: false } }));
      ...
      if (message.type === 'tts.commit') {
        ...
        socket.send(JSON.stringify({ type: 'tts.started', payload: { streamId, responseId: message.payload.responseId, epoch: message.payload.epoch, playbackId, outputStreamId, sampleRate: 24000, voiceId } }));
        setTimeout(() => {
          for (let sequence = 0; sequence < 3; sequence++) {
            socket.send(encodeBinaryAudioFrame({ channel: 2, streamId: outputStreamId, sequence, monotonicUs: BigInt(sequence + 1), pcm16: new Int16Array(480) }, 64 * 1024));
          }
          socket.send(JSON.stringify({ type: 'tts.ended', payload: { streamId, responseId: message.payload.responseId, epoch: message.payload.epoch, playbackId, generatedSamples: 1440 } }));
        }, 20);
      }
    });
  });
...
describe('synthesizeVoicePreview', () => {
  ...
  it('rejects when the sidecar refuses a second stream (session already active)', async () => {
    ...
    await expect(synthesizeVoicePreview(sidecar, { catalogId: 'sess-catalog', voiceId: 'af_heart', phrases: ['Hello there!'] })).rejects.toThrow();
  ...
  it('fails closed when the sidecar echoes a different voice than requested', async () => {
    ...
    await expect(synthesizeVoicePreview(sidecar, { catalogId: 'sess-catalog', voiceId: 'af_bella', phrases: ['Hello there!'] })).rejects.toThrow(/protocol/);
  });

  it('fails closed when the verified catalog drifts from the preview selection', async () => {
    const { sidecar } = await fakePreviewSidecar();
    await expect(synthesizeVoicePreview(sidecar, { catalogId: 'other-catalog', voiceId: 'af_heart', phrases: ['Hello there!'] })).rejects.toThrow(/catalog|not ready/);
  });

  it('aborts cleanly when the caller cancels before completion', async () => {
    const { sidecar } = await fakePreviewSidecar();
    const controller = new AbortController();
    const pending = synthesizeVoicePreview(sidecar, { catalogId: 'sess-catalog', voiceId: 'af_heart', phrases: ['Hello there!'] }, { signal: controller.signal });
    controller.abort();
