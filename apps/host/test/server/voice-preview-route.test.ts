...
const qwenCatalog = Object.freeze({
...
async function fakeSidecarHealth(options: {
  ready: boolean;
  qwen?: boolean;
  failHealthRequests?: number[];
  delayHealthRequests?: Record<number, number>;
}) {
...
afterEach(async () => {
  ...
  for (const close of closed.splice(0)) await close;
});

async function makeApp(
  voicePreview: PreviewStub,
  ready: boolean = true,
  qwen: boolean = false,
  options: {
    probeClient?: PiClient;
    now?: () => number;
    sidecarFailHealthRequests?: number[];
    sidecarDelayHealthRequests?: Record<number, number>;
  } = {},
): Promise<{ app: FastifyInstance; origin: string; sidecarHealthRequests: () => number }> {
  const { probeClient, sidecarFailHealthRequests, sidecarDelayHealthRequests, ...buildOptions } = options;
  const { sidecar, healthRequests, close } = await fakeSidecarHealth({
    ready,
    qwen,
    failHealthRequests: sidecarFailHealthRequests,
    delayHealthRequests: sidecarDelayHealthRequests,
  });
  closed.push(close);
  const app = await buildApp({
    sidecar,
    voicePreview,
    createProbeClient: () => probeClient ?? defaultProbeClient,
    ...buildOptions,
  });
  apps.push(app);
...
async function preview(
  origin: string,
  auth: { capability: string; cookie: string; headers: Record<string, string> },
  voiceId: string,
) {
  return fetch(`${origin}/api/voice-preview`, {
    method: 'POST',
    headers: {
      ...auth.headers,
      cookie: auth.cookie,
      'x-podcaster-capability': auth.capability,
...
describe('POST /api/readiness', () => {
    ...
    const { origin } = await makeApp(
      ...
      false,
      {
        probeClient: {
          ...
          async probe() {
            factoryCalls++;
            return defaultProbeClient.probe();
          },
        },
      },
    );
    ...
    const pi: PiClient = {
      async probe() {
        ...
        return new Promise((resolve) => {
          releaseRefresh = () => resolve({ status: 'ready', detail: 'Pi is ready.', correctiveAction: 'None.' });
        });
      },
      async *request() {},
      async shutdown() {},
  ...
  it('requires two consecutive Pi failures before publishing the latest downgrade', async () => {
    ...
    const pi: PiClient = {
      async probe() {
        ...
        if (probeCount === 1) return { status: 'ready', detail: 'Pi is ready.', correctiveAction: 'None.' };
        if (probeCount === 2)
          return { status: 'unavailable', detail: 'Pi is unavailable.', correctiveAction: 'Retry.' };
  ...
  it('reports the selected Qwen CustomVoice backend as the active ready backend', async () => {
    ...
    const body = (await response.json()) as {
      ...
      sidecar: string;
      activeTtsModel?: { backendId: string; modelId: string };
    };
  ...
  it('keeps the verified Kokoro catalog visible when an old sidecar lacks model descriptors', async () => {
    const { origin } = await makeApp(async (input) => ({
      pcm16: new Int16Array(input.phrases.length),
      sampleRate: 24_000,
    }));
    const auth = await bootstrap(origin);
    const response = await fetch(`${origin}/api/readiness`, {
...
describe('POST /api/voice-preview', () => {
  it('returns a playable WAV for a verified voice with three randomized phrases', async () => {
    ...
    const { origin } = await makeApp(async (input) => {
      captured.push(input);
      return { pcm16: new Int16Array([100, -200, 300, -400]), sampleRate: 24_000 };
    });
    const auth = await bootstrap(origin);
  ...
  it('requires an authenticated session', async () => {
    ...
    const response = await fetch(`${origin}/api/voice-preview`, {
      method: 'POST',
      headers: h,
      body: JSON.stringify({ voiceId: 'af_heart' }),
    });
  ...
  it('rejects a malformed body and an unknown voice', async () => {
    ...
    expect(
      (await fetch(`${origin}/api/voice-preview`, { method: 'POST', headers: h, body: JSON.stringify({}) })).status,
    ).toBe(400);
    expect(
      (
        await fetch(`${origin}/api/voice-preview`, {
          method: 'POST',
          headers: h,
          body: JSON.stringify({ voiceId: 'nope' }),
        })
      ).status,
    ).toBe(422);
  });
