...
function command(sessionId: string, type: string, payload: TestJsonRecord, epoch = 0) {
  return {
    ...
    sessionId,
    epoch,
    eventId: `018f1f32-7abf-7def-8abc-${(0x1000 + sequence++).toString(16).padStart(12, '0')}`,
    type,
...
class TrackingPi implements PiClient {
  ...
  shutdownCalls = 0;
  constructor(personaAppend: string, piSettings?: PiSettings) {
    this.personaAppend = personaAppend;
...
class TrackingResearch implements PiResearchClient {
  ...
  async *requestBody(): AsyncIterable<PiEvent> {
    yield { type: 'final', text: 'ok' };
  }
  async shutdown() {
    this.shutdownCalls++;
...
interface SessionHarness {
  ...
  close: () => Promise<void>;
  response: TrackingPi[];
  research: TrackingResearch[];
  classifier: TrackingPi[];
}
async function build(pi: PiClient): Promise<SessionHarness> {
  ...
  const app = await buildApp({
    ...
    createResponseClient: (append, piSettings) => {
      const client = new TrackingPi(append, piSettings);
      response.push(client);
      return client;
    },
    createResearchClient: (append, piSettings) => {
      const client = new TrackingResearch(append, piSettings);
      research.push(client);
      return client;
    },
    createClassifierClient: (piSettings) => {
      const client = new TrackingPi('', piSettings);
      classifier.push(client);
      return client;
    },
  });
...
async function bootstrapSession(app: FastifyInstance, origin: string, persona: string, sid: string) {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    socket.once('message', (raw) =>
      raw.toString().includes('authenticated') ? resolve() : reject(new Error('not authenticated')),
    );
  });
  socket.send(
    JSON.stringify(
      command(sid, 'session.open', {
        ...
        settings: {
          ...
          persona,
          voice: VOICE,
          pi: { model: 'openai-codex/gpt-5.6-sol', thinkingLevel: 'high' },
        },
      }),
    ),
  );
...
describe('per-session Pi isolation', () => {
  it('creates separate response/research/classifier clients per persona and shuts them all down on stop', async () => {
    const probe: PiClient = {
      async probe() {
        return { status: 'ready', detail: '', correctiveAction: 'None.' };
      },
      async *request() {
        yield { type: 'final', text: 'ok' };
      },
    ...
    expect(
      [...harness.response, ...harness.research, ...harness.classifier].every(
        (client) => client.piSettings?.thinkingLevel === 'high',
      ),
    ).toBe(true);
