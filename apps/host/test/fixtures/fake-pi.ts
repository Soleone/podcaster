...
export async function makeFakePi(scenario: FakePiScenario = "normal", version = "0.84.0"): Promise<FakePi> {
...
function command(c) {
  ...
  if (c.type === "abort") { aborted = true; send({type:"message_end", message:{role:"assistant", stopReason:"aborted", errorMessage:"aborted"}}); send({type:"agent_settled"}); later(() => send({ type:"response", id:c.id, command:c.type, success:true }), 2); return; }
