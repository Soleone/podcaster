...
const input: PiRequestInput = {
  posture: 'question',
  transcript: 'What should we test next?',
  boundedContext: '',
  maxWords: 45,
};
...
async function setup() {
  const fake = await makeFakePi('slow');
  cleanups.push(fake.cleanup);
  return {
    fake,
    client: new StdioPiClient({ executable: fake.executable, startupDeadlineMs: 300, requestDeadlineMs: 600 }),
  };
}
async function collect(client: StdioPiClient, signal: AbortSignal, onEvent?: (event: PiEvent) => void) {
  const found: PiEvent[] = [];
  for await (const event of client.request(input, signal)) {
    found.push(event);
    onEvent?.(event);
  }
  return found;
