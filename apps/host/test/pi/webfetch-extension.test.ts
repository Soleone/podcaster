...
describe('research webfetch extension', () => {
  it('registers one bounded tool and returns readable untrusted text', async () => {
    const fetchMock = vi.fn(
      async () =>
        new Response(
          '<html><head><script>ignore me</script></head><body><h1>Release dates</h1><p>New game on Friday.</p></body></html>',
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      new URL('https://example.test/releases'),
  ...
  it('rejects non-web schemes and oversized responses without shell access', async () => {
    ...
    const tool = await registeredTool();
    await expect(tool.execute('call-2', { url: 'file:///etc/passwd' }, new AbortController().signal)).rejects.toThrow(
      'only permits http and https',
    );
    await expect(
      tool.execute('call-3', { url: 'https://example.test/large' }, new AbortController().signal),
