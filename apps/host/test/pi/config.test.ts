...
describe("Pi executable resolution", () => {
  ...
  it("rejects relative and non-canonical explicit paths", async () => {
    ...
    expect(() => resolvePiExecutable(environment({ PODCASTER_PI_EXECUTABLE: "./pi", PATH: "/private/secret" }))).toThrow(/absolute/iu);
  ...
  it("rejects absent or non-executable Pi without exposing environment values", async () => {
    ...
    expect(() => resolvePiExecutable(environment({ PODCASTER_PI_EXECUTABLE: missingPath, PATH: "/private/token-value" }))).toThrow(/executable file/iu);
    let caught: unknown;
    try {
