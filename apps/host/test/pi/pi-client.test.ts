...
describe("production Pi RPC boundary", () => {
  ...
  it.each(["malformed", "oversized", "invalid-utf8", "crlf", "crash", "too-many-words"] as const)("fails safely on %s child output", async (scenario) => {
    const { value } = await client(scenario);
    const result = await events(value);
  ...
  it.each([["async-login", "login_required"], ["async-rate-limit", "rate_limited"]] as const)("classifies sanitized asynchronous provider failure %s", async (scenario, state) => {
    const { value } = await client(scenario);
    const result = await events(value);
