...
class FakeSpeech implements SpeechOutputPort {
  ...
  synthesize(input: { responseId: string; partIndex?: number; text: string; signal?: AbortSignal }): Promise<{
    playbackId: string;
    sampleRate: number;
...
function turn(index: number, text = 'What is the capital of France and why does it matter?', epoch = 0) {
...
describe('safe session orchestrator multi-part', () => {
  it('emits stall part 0 before body parts with correct event ordering', async () => {
    const body = async function* (): AsyncIterable<PiEvent> {
      yield {
        type: 'delta',
        text: 'The capital of France is Paris. It has been the capital for centuries. The city sits on the Seine river. It is a major cultural center. The metro system is extensive. Millions visit every year. ',
      };
      yield {
        type: 'final',
        text: 'The capital of France is Paris. It has been the capital for centuries. The city sits on the Seine river. It is a major cultural center. The metro system is extensive. Millions visit every year. ',
    ...
    const indexOf = (type: string, payload?: Record<string, unknown>) =>
      events.findIndex(
        (event) =>
          event.type === type &&
          (!payload || Object.entries(payload).every(([key, value]) => event.payload[key] === value)),
      );
  ...
  it('adds the concatenated parent text to context once after all parts complete', async () => {
    ...
    for (const begin of speech.begins) {
      session.playbackStopped({
        playbackId: ids[81 + speech.begins.indexOf(begin)]!,
        cancelledEpoch: 0,
  ...
  it('keeps the streamed stall prefix when the stall request errors after emitting sentence chunks', async () => {
    ...
    const { session, events, speech, researchPi } = setup({
      pi: new FakePi(stallThenError),
      researchPi: new FakeResearchPi(body),
    });
    const handling = session.handleStableFinal(turn(0));
    await handling;
