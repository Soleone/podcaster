...
import {
  InvalidPartError,
  RESEARCH_PART_LIMITS,
  ResearchPartAssembler,
} from '../../src/session/ResearchPartAssembler.js';

function assembleAppendThenFinal(
  deltas: string[],
  options?: { maxPartWords?: number; maxPartSentences?: number; maxParts?: number },
) {
  const assembler = new ResearchPartAssembler(
    options?.maxPartWords,
    4096,
    options?.maxPartSentences,
    options?.maxParts,
  );
  let released: string[] = [];
