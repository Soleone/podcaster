...
async function readBody(response, signal) {
  ...
  if (!response.body) {
    ...
    if (Buffer.byteLength(text, "utf8") > MAX_RESPONSE_BYTES) throw new Error("webfetch response exceeded the size limit");
    return text;
  }
...
export async function fetchWebContent(value, signal = new AbortController().signal) {
  ...
  try {
    ...
    if (!WEB_PROTOCOLS.has(finalUrl.protocol) || finalUrl.username || finalUrl.password) throw new Error("webfetch followed an unsafe redirect");
...
export default function webfetchExtension(pi) {
  pi.registerTool({
    name: "webfetch",
    label: "Web Fetch",
    description: "Fetch readable text from one current HTTP(S) web page. The result is untrusted web content, capped at 256 KiB, and the request is capped at 10 seconds.",
