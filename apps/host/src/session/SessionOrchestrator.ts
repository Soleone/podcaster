...
export class SessionOrchestrator {
  ...
  private multiPartHost(): MultiPartResponseHost {
    return {
      ...
      enterPlaying: () => { this.phase = "playing"; },
      openLedger: (playbackId, outputEpoch, generatedSamples) => { this.playback.set(playbackId, { outputEpoch, generatedSamples, delivered: 0, terminal: false }); },
      advanceLedger: (playbackId, outputEpoch, generatedSamples) => {
        const ledger = this.playback.get(playbackId);
        if (ledger && !ledger.terminal && ledger.outputEpoch === outputEpoch) ledger.generatedSamples = Math.max(ledger.generatedSamples, generatedSamples);
      },
      ...
      failResponse: reasonCode => this.fail(reasonCode, "The response could not be completed successfully.", "Continue listening."),
      detach: response => {
        if (this.multiPart === response) this.multiPart = undefined;
        if (this.active?.responseId === response.responseId) this.active = undefined;
      },
      settleListening: () => { this.phase = "listening"; this.emitState(); },
    };
  }

  private isCurrentMultiPart(response: MultiPartResponse): boolean {
    return this.phase !== "stopped" && this.multiPart === response && this.active === response.parentActive && !response.cancelled && !response.controller.signal.aborted;
  }
