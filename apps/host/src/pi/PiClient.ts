...
export class StdioPiClient implements PiClient {
  ...
  private active: PiActiveRequest | undefined; private probeLifecycle: Lifecycle | undefined;
  ...
  constructor(options: PiClientOptions = {}) {
    ...
    this.process = new PiRpcProcess({
      ...
      args: ["--mode", "rpc", "--no-session", "--no-tools", "--no-extensions", "--no-skills", "--no-prompt-templates", "--no-context-files", "--no-approve", "--model", this.model, ...(this.thinkingLevel ? ["--thinking", this.thinkingLevel] : []), "--system-prompt", this.systemPrompt, "--append-system-prompt", [this.personaAppend, RESPONSE_ONLY_SYSTEM_INSTRUCTION].filter(Boolean).join("\n\n")],
  ...
  private async beginRequest(input: PiRequestInput, signal: AbortSignal, queue: AsyncQueue<PiEvent>, isCancelled: () => boolean): Promise<void> {
    ...
    try {
      ...
      if (signal.aborted || isCancelled()) { queue.end(); release(); return; }
      const active: PiActiveRequest = { queue, cutoff: false, assistantText: "", responseBytes: 0, textExceeded: false, messageEnded: false, stopReason: undefined, providerError: undefined, settled: false, completed: false,
  ...
  private finishActive(active: PiActiveRequest, release = true): void { if (active.completed) return; active.completed = true; clearTimeout(active.timer); active.signal.removeEventListener("abort", active.abortListener); if (this.active === active) this.active = undefined; if (release) active.release(); }
