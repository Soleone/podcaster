...
export async function buildApp(options: BuildOptions): Promise<FastifyInstance> {
  ...
  const voicePreview = options.voicePreview ?? (async (input: { catalogId: string; voiceId: string; speedModifier?: number; tonePrompt?: string; language?: QwenVoiceLanguage; backendId?: string; modelId?: string; phrases: string[] }, signal: AbortSignal) => {
  ...
  app.post('/api/voice-preview', { schema: { body: { type: 'object', additionalProperties: false, required: ['voiceId'], properties: { voiceId: { type: 'string', minLength: 1, maxLength: 128 }, catalogId: { type: 'string', minLength: 1, maxLength: 128 }, backendId: { type: 'string', minLength: 1, maxLength: 128 }, modelId: { type: 'string', minLength: 1, maxLength: 256 }, speedModifier: { type: 'number', minimum: MIN_VOICE_SPEED_MODIFIER, maximum: MAX_VOICE_SPEED_MODIFIER }, tonePrompt: { type: 'string', minLength: 1, maxLength: MAX_VOICE_TONE_PROMPT_BYTES }, language: { enum: [...QWEN_VOICE_LANGUAGES] } } } } }, async (request, reply) => {
    ...
    if (language !== undefined && backendId !== 'qwen3') return reply.code(422).send({ error: 'unsupported_language' });
    ...
    try {
      const { pcm16, sampleRate } = await voicePreview({ catalogId: selectedCatalog.catalogId, voiceId, speedModifier, ...(normalizedTonePrompt ? { tonePrompt: normalizedTonePrompt } : {}), ...(language ? { language } : {}), backendId, modelId, phrases: randomVoicePreviewPhrases() }, controller.signal);
